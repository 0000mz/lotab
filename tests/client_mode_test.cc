...
// -----------------------------------------------------------------------------
TEST_F(ClientModeTest, EscapeCancelsFilter) {
    LmModeTransition tx;
    LmMode old_mode, new_mode;
    
    // 1. Start filtering
    lm_process_key_event(ctx, 44, '/', 0, 0, &tx, &old_mode, &new_mode);
    ASSERT_EQ(tx, LM_MODETS_ADHERE_TO_MODE);
    ASSERT_EQ(new_mode, LM_MODE_LIST_FILTER_INFLIGHT);
    
    // 2. Type "abc"
    lm_process_key_event(ctx, 'a', 'a', 0, 0, &tx, &old_mode, &new_mode);
    lm_process_key_event(ctx, 'b', 'b', 0, 0, &tx, &old_mode, &new_mode);
    lm_process_key_event(ctx, 'c', 'c', 0, 0, &tx, &old_mode, &new_mode);
    ASSERT_EQ(tx, LM_MODETS_UPDATE_LIST_FILTER);
    
    // 3. Press ESC (Cancel)
    // Expect transition to LIST_NORMAL *and* empty filter
    lm_process_key_event(ctx, 53, 0, 0, 0, &tx, &old_mode, &new_mode);
    ASSERT_EQ(tx, LM_MODETS_ADHERE_TO_MODE);
    ASSERT_EQ(new_mode, LM_MODE_LIST_NORMAL);
    

    
    // Verify filter is cleared
    const char* ft = lm_get_filter_text(ctx);
    EXPECT_TRUE(ft == NULL || strlen(ft) == 0);
    
    // 4. Press ESC again
    // Expect HIDE_UI (since no filter)
    lm_process_key_event(ctx, 53, 0, 0, 0, &tx, &old_mode, &new_mode);
    ASSERT_EQ(tx, LM_MODETS_HIDE_UI);
}
