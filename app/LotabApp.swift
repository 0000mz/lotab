...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    source.setEventHandler {
      vlog_s(.info, LotabApp.appClass, "DispatchSource: SIGTERM received")
      NSApp.terminate(nil)
    }
    source.resume()
  ...
  func handleTransition(
    ...
    case LM_MODETS_ACTIVATE_TO_TAB:
      ...
      if let selectedId = Lotab.shared.selection {
        if let client = self.udsClient {
          lotab_client_send_tab_selected(client, Int64(selectedId))
        }
        self.hideUI()
        return nil
      }
    ...
    case LM_MODETS_COMMIT_LIST_FILTER:
      tm.isFiltering = false
      tm.selection = tm.displayedTabs.first?.id
      break
    ...
    case LM_MODETS_ASSOCIATE_TASK:
      ...
      if selection > 0 && selection - 1 < tm.tasks.count {
        ...
        if !idsToAssoc.isEmpty, let client = self.udsClient {
          let cIds = idsToAssoc.map { Int64($0) }
          cIds.withUnsafeBufferPointer { buffer in
            lotab_client_send_associate_tabs(
              client, buffer.baseAddress, Int(buffer.count), Int64(taskId))
          }
        }
      }
      tm.isAssociatingTask = false
  ...
  private func showUI() {
    DispatchQueue.main.async {
      ...
      // Reset selection to first item to ensure consistent navigation start
...
class Lotab: ObservableObject {
  ...
  public func closeSelectedTabs(udsClient: OpaquePointer?) {
    ...
    if !idsToClose.isEmpty {
      ...
      if let client = udsClient {
        let cIds = idsToClose.map { Int64($0) }
        cIds.withUnsafeBufferPointer { buffer in
  ...
  public func dumpManifest() {
    guard let path = self.manifestPath else { return }
    let tabsData = self.tabs.map {
      [
        "id": $0.id,
        "title": $0.title,
        "active": $0.active,
    ...
    do {
      ...
      vlog_s(.info, LotabApp.appClass, "Dumped GUI manifest to \(url.path)")
    } catch {
      vlog_s(.error, LotabApp.appClass, "Failed to dump manifest: \(error)")
    }
  }
}
...
struct LotabApp: App {
  ...
  init() {
    ...
    if let index = args.firstIndex(of: "--gui-manifest-path"), index + 1 < args.count {
      Lotab.shared.manifestPath = args[index + 1]
    }
  }
